...
class PageRegistry extends SingletAssocArrayWrapper {
  ...
  private static $commonParams=[
    ...
    "realm"=>"redefine this in .ini",
    "roomId"=>"redefine this in .ini",
    "maxPublisherCount"=>5,
    "wsServerPort"=>8989,
    "password"=>"",
    "mediaFolder"=>"share",
  ...
  public function exportByList($keys,Array $target) {
    $r=$target;
    foreach($keys as $k) { 
      $r[$k]=$this->arr[$k];
  ...
  public function dump() { print_r($this->arr); }
}
...
class IniParams {
  ...
  public static function read($localPath) {
    ...
    if( ! file_exists($localPath.self::$name)) echo("Failed to find file:".$localPath.self::$name."!");
    $buf="";
    $r=[];
